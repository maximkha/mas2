...
namespace MaxAmpS2
{
	class MainClass
	{
		...
		{
			int p;
			if (!Int32.TryParse(args[0], out p)) { ShowUsage(); return;}
			if (!Directory.Exists(args[1])) { ShowUsage(); return;}
			//if (!File.Exists(args[2])) { ShowUsage(); return; }
			if(!(args[2].IndexOfAny(Path.GetInvalidFileNameChars()) >= 0)){ ShowUsage(); return; }

			Logpath = args[2];

			MaxAmp2.MAS2 MyAmp = new MaxAmp2.MAS2(IPAddress.Any,p);
			MyAmp.RootPath = args[1];

			MyAmp.RegisterErrorHandler(Handle, "S");
			MyAmp.RegisterErrorHandler(Handle, "RC");
			MyAmp.RegisterErrorHandler(Handle, "HC");
			MyAmp.RegisterErrorHandler(Handle, "PU");
			MyAmp.RegisterErrorHandler(Handle, "HFR");
			MyAmp.RegisterErrorHandler(Handle, "GD");
			MyAmp.RegisterErrorHandler(Handle, "GFC");
			MyAmp.RegisterErrorHandler(Handle, "PP");
			MyAmp.RegisterErrorHandler(Handle, "STF");
			MyAmp.RegisterErrorHandler(Handle, "RF");
			MyAmp.RegisterErrorHandler(Handle, "GE");
			MyAmp.RegisterErrorHandler(Handle, "GH");
			MyAmp.RegisterErrorHandler(Handle, "ST");

            MyAmp.RegisterVirtDir(writeFile, "/dump");
            path = Path.GetDirectoryName(Logpath)+"/dump.txt";

            MyAmp.Start();
		}

        public static string writeFile(MaxAmp2.MAS2.PrevReq pr)
        {
            if (!pr.isPost) return "<head><script src='stream.js'></script></head><body>POST INTERFACE</body>";
            string data = pr.postParams["data"];
            data = data.Replace("/n", '\n'.ToString());
            if (!File.Exists(path))
            {
		...
		public static int Handle(MaxAmp2.ErrorArg EA)
		{
			...
			try
			{
				...
				{
					file.WriteLine("===============================");
					file.WriteLine("Severity:" + EA.Severity);
					file.WriteLine("Location:" + EA.Location);
					file.WriteLine("Date:" + EA.TimeAndDate);
					file.WriteLine("ERROR: ");
					file.WriteLine(EA.Error);
					file.WriteLine("===============================");
				}
			}
			catch (Exception E)
		...
		{
			Console.WriteLine("MaxAmpS2.exe <Port>,<Path>,<LogPath>");
			return;
		}
	}
	...
	namespace MaxAmp2{
		...
		*/
		...
		public class MAS2
		{
			...
			public static string[] indexfiles = { "index.html", "index.php" };
